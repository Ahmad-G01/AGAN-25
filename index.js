...
const instagram = 'https://instagram.com/ahmad_gustian_adi_nugraha'; // Nama Instagramlu cok
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @IAN_25ID`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @IAN_25ID`)
})
...
//Tolonglah bro jangan di ubah ubah GUSTIAN BOT
